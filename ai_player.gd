...
func _make_ceo_decision():
	var candidates = _get_available_candidates()
	if candidates.is_empty():
		return

	for candidate in candidates:
		var offer_amount = _calculate_offer_amount(candidate)
		if offer_amount > 0 and offer_amount <= budget:
			game_instance.make_offer(player_id, candidate.player_id, offer_amount)
			budget -= offer_amount
			break

func _make_candidate_decision():
	var offers = game_instance.get_offers_for_candidate(player_id)
	if offers.is_empty():
		return

	var best_offer = _evaluate_best_offer(offers)
	if best_offer:
		game_instance.accept_offer(player_id, best_offer.ceo_id, best_offer.amount)

func _get_available_candidates():
	return game_instance.get_available_candidates()

func _calculate_offer_amount(candidate):
...
func _evaluate_best_offer(offers):
	var best_offer = null
	var best_value = 0
	
	for offer in offers:
		var offer_value = _calculate_offer_value(offer)
...
func _calculate_offer_value(offer):
